    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onClick(View v) {
                try{
                    if(!etUpdateSno.getText().toString().equals("")) {
                        //先判断是否存在该学生
                        Cursor isExists = sqRead.query(Student.TABLE, new String[]{Student.KEY_SNO},
                                Student.KEY_SNO + "=?", new String[]{etUpdateSno.getText().toString()}, null, null, null);
                        if (isExists.getCount() == 0)
                            ...
                            Cursor c = sqRead.query(Student.TABLE, null, Student.KEY_SNO + "=?",
                            ...
                            if (c.getBlob(4) != null) {
                                ...
                                ivUpdateTouXiang.setImageResource(R.mipmap.ic_launcher);
                        }
                    }
                    else
        ...
        btnUpdateData.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                cv.put(Student.KEY_SNO,etUpdateSno.getText().toString());
                cv.put(Student.KEY_SNAME,etUpdateSname.getText().toString());
                cv.put(Student.KEY_SCLASS,etUpdateSclass.getText().toString());
                cv.put(Student.KEY_SBEIZHU,date+etUpdateCaoXing.getText().toString());
                if (isUpdateTouXiang == true) {
                    ivUpdateTouXiang.setDrawingCacheEnabled(true); //能够获取ImageView中的图片
                    cv.put(Student.KEY_STOUXIANG, getPicture(updataBitmap));
                    ivUpdateTouXiang.setDrawingCacheEnabled(false);
                }
                isUpdateTouXiang = false;
                sqWrite.update(Student.TABLE,cv,Student.KEY_SNO+"=?",new String[]{strUpdateSno});
